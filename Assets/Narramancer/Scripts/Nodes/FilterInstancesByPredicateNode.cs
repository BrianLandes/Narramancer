...
namespace Narramancer {
	...
	[NodeWidth(250)]
	[CreateNodeMenu("Noun/Filter Instances")]
	public class FilterInstancesByPredicateNode : Node {
		...
		public override void UpdatePorts() {
			...
			if (predicate == null) {
				...
				var inputPort = predicate.GetInput<NounInstance>();

				foreach (var inputGraphPort in predicate.Inputs) {
					if (inputGraphPort.Type == null) {
						continue;
					}
					// don't add an input port for the Instance input -> we'll assign that one manually
					if (inputGraphPort == inputPort) {
						continue;
					}
		...
		private bool DoesInstancePassPredicate(object context, NounInstance instance) {
			AssignGraphVariableInputs(context);

			return predicate.RunForValue<bool, NounInstance>(context, instance);
		}

		private List<NounInstance> GetFilteredInstances(object context) {
